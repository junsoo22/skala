...
| 3) 예측력에 기여하는 요인 파악 | `LogisticRegression.coef_`를 피처 이름과 매칭해 분석 완료 (아래 표) | ✅ 달성 |

**종합 평가**: 4개 KPI 모두 달성했다. 3번 목표는 `ml_pipeline._feature_importance()`로
`coef_`와 `ColumnTransformer.get_feature_names_out()`을 매칭해 확인했다.

**안정적인 피처(전체 응답자에 존재, 표본 걱정 없음) 기준 상위 요인**:

| 피처 | 계수 | 해석 |
|---|---|---|
| `lang_typescript` | +0.126 | TypeScript 사용자는 재택근무 가능성 ↑ |
| `YearsCode` | +0.125 | 코딩 경력이 길수록 재택근무 가능성 ↑ |
| `lang_csharp` | -0.122 | C# 사용자는 재택근무 가능성 ↓ |
| `lang_java` | -0.112 | Java 사용자는 재택근무 가능성 ↓ |
| `WorkExp` | +0.095 | 실무 경력이 길수록 재택근무 가능성 ↑ |
| `ConvertedCompYearly` | +0.033 | 연봉이 높을수록 재택근무 가능성 ↑ (다른 요인 통제 시 효과는 작음) |

**해석**: TypeScript/JavaScript 진영(웹·스타트업 문화권)은 재택 성향이 강하고, C#/Java 진영
(엔터프라이즈·SI 문화권)은 출근 성향이 강하다는, 실무적으로 납득 가능한 패턴이 나왔다. 경력이
길수록(`YearsCode`/`WorkExp`) 재택 가능성이 높은 것도 "연차가 쌓일수록 재택 협상력이
높아진다"는 통념과 맞아떨어진다.

**주의**: 원-핫 인코딩된 범주형 피처(국가 등)는 표본이 적은 범주(예: 응답자 16명뿐인 나라)일
수록 계수가 커지는 과적합 경향이 있어, `MIN_CATEGORY_SAMPLES=500` 이상인 것만 신뢰할 수 있는
것으로 걸러서 봤다 (`report.md` 5.1절 참고). 그 기준으로도 국가별 계수(우크라이나 +2.08,
스위스 -1.35 등)가 가장 크게 나오는데, 이는 국가별 재택근무 문화·법제 차이가 개인 특성보다
훨씬 강하게 작용한다는 뜻으로 해석할 수 있다 — 다만 인과관계가 아니라 상관관계라는 점은
유의해야 한다.
